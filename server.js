...
/* ============================================================
   Signaux internes multi-timeframes
   Objectif : fournir une couche indépendante des indicateurs privés TradingView.
   Ces signaux ne reproduisent pas le code de Gold signal ou Groupe privé V2.
   Ils proposent une approximation technique contrôlée par le serveur.
============================================================ */

function normaliserDecisionInterne(decision) {
    const valeur = String(decision || "attendre").toLowerCase();
    if (["acheter", "achat", "buy", "long"].includes(valeur)) return "acheter";
    if (["vendre", "vente", "sell", "short"].includes(valeur)) return "vendre";
    return "attendre";
}

function construireGoldSignalInterne(analyse = {}) {
    const prix = Number(analyse.prix_actuel);
    const ema20 = Number(analyse.ema20);
    const ema50 = Number(analyse.ema50);
    const rsiValeur = Number(analyse.rsi);
    const histogramme = Number(analyse.macd?.histogramme);
    ...
    if (
        ...
        decision = "acheter";
        confiance = 68;
        raisons.push("Prix au-dessus de l'EMA20, EMA20 au-dessus de l'EMA50, RSI positif sans excès extrême.");
    }
    ...
    if (
        ...
        decision = "vendre";
        confiance = 68;
        raisons.push("Prix sous l'EMA20, EMA20 sous l'EMA50, RSI négatif sans excès extrême.");
    }

    if (decision !== "attendre" && Number.isFinite(histogramme)) {
        if ((decision === "acheter" && histogramme > 0) || (decision === "vendre" && histogramme < 0)) {
            confiance += 8;
            raisons.push("MACD cohérent avec le sens du signal.");
        } else {
            confiance -= 10;
            raisons.push("MACD insuffisamment cohérent avec le sens du signal.");
        }
    }

    if (!raisons.length) {
        raisons.push("Conditions internes insuffisantes pour valider un signal Gold-like.");
    }

    return {
        nom: "Gold signal interne",
        decision,
        confiance: Math.max(0, Math.min(95, confiance)),
        raisons
    };
}

function construireGroupePriveV2Interne(analyse = {}) {
    const prix = Number(analyse.prix_actuel);
    const support = Number(analyse.support_principal);
    const resistance = Number(analyse.resistance_principale);
    const ema20 = Number(analyse.ema20);
    const ema50 = Number(analyse.ema50);
    const ema200 = Number(analyse.ema200);
    const rsiValeur = Number(analyse.rsi);
    const histogramme = Number(analyse.macd?.histogramme);
    ...
    if (Number.isFinite(prix) && Number.isFinite(ema20)) {
        if (prix > ema20) {
            score += 1;
            raisons.push("Prix supérieur à l'EMA20.");
        } else {
            score -= 1;
            raisons.push("Prix inférieur à l'EMA20.");
        }
    }
    ...
    if (Number.isFinite(ema20) && Number.isFinite(ema50)) {
        if (ema20 > ema50) {
            score += 1;
            raisons.push("EMA20 au-dessus de l'EMA50.");
        } else {
            score -= 1;
            raisons.push("EMA20 sous l'EMA50.");
        }
    }
    ...
    if (Number.isFinite(ema50) && Number.isFinite(ema200)) {
        if (ema50 > ema200) {
            score += 1;
            raisons.push("Structure EMA50/EMA200 positive.");
        } else {
            score -= 1;
            raisons.push("Structure EMA50/EMA200 négative.");
        }
    }

    if (Number.isFinite(histogramme)) {
        if (histogramme > 0) {
            score += 1;
            raisons.push("Histogramme MACD positif.");
        } else if (histogramme < 0) {
            score -= 1;
            raisons.push("Histogramme MACD négatif.");
        }
    }

    if (Number.isFinite(rsiValeur)) {
        if (rsiValeur > 55 && rsiValeur < 75) score += 1;
        if (rsiValeur < 45 && rsiValeur > 25) score -= 1;
        raisons.push("RSI : " + rsiValeur + ".");
    }

    if (Number.isFinite(prix) && Number.isFinite(resistance) && prix >= resistance * 0.995) {
        score -= 1;
        raisons.push("Prix proche d'une résistance : prudence côté achat.");
    }

    if (Number.isFinite(prix) && Number.isFinite(support) && prix <= support * 1.005) {
        score += 1;
        raisons.push("Prix proche d'un support : possible réaction technique.");
    }

    let decision = "attendre";
    if (score >= 3) decision = "acheter";
    if (score <= -3) decision = "vendre";

    return {
        nom: "Groupe privé V2 interne",
        decision,
        score,
        confiance: Math.max(35, Math.min(90, 45 + Math.abs(score) * 9)),
        raisons
    };
}

function fusionnerDecisionsInternes(gold, groupe) {
    const d1 = normaliserDecisionInterne(gold?.decision);
    const d2 = normaliserDecisionInterne(groupe?.decision);

    if (d1 === d2 && d1 !== "attendre") {
        return {
            decision: d1,
            confiance: Math.round(((gold.confiance || 50) + (groupe.confiance || 50)) / 2)
        };
    }

    if (d1 !== "attendre" && d2 === "attendre") {
        return {
            decision: d1,
            confiance: Math.max(40, Math.round((gold.confiance || 50) * 0.82))
        };
    }

    if (d2 !== "attendre" && d1 === "attendre") {
        return {
            decision: d2,
            confiance: Math.max(40, Math.round((groupe.confiance || 50) * 0.82))
        };
    }
...
app.post("/api/signaux-internes-mtf", async (req, res) => {
    try {
        const body = req.body || {};
        const timeframes = Array.isArray(body.timeframes) && body.timeframes.length
            ? body.timeframes
            : [
                { key: "m1", label: "M1", intervalle: "1", actif: body.actif || "BINANCE:BTCUSDT" },
                { key: "m5", label: "M5", intervalle: "5", actif: body.actif || "BINANCE:BTCUSDT" },
                { key: "m15", label: "M15", intervalle: "15", actif: body.actif || "BINANCE:BTCUSDT" },
                { key: "m30", label: "M30", intervalle: "30", actif: body.actif || "BINANCE:BTCUSDT" },
                { key: "m45", label: "M45", intervalle: "45", actif: body.actif || "BINANCE:BTCUSDT" },
                { key: "h1", label: "H1", intervalle: "60", actif: body.actif || "BINANCE:BTCUSDT" }
            ];

        const signaux = [];

        for (const timeframe of timeframes.slice(0, 6)) {
            const actif = timeframe.actif || body.actif || "BINANCE:BTCUSDT";
            const intervalle = String(timeframe.intervalle || "60");

            try {
                const marche = await recupererBougiesMarche(actif, intervalle);
                const technique = analyseTechnique({ actif, intervalle, marche });
                const gold = construireGoldSignalInterne(technique);
                const groupe = construireGroupePriveV2Interne(technique);
                const fusion = fusionnerDecisionsInternes(gold, groupe);

                signaux.push({
                    ok: true,
                    key: timeframe.key || intervalle,
                    label: timeframe.label || intervalle,
                    actif,
                    intervalle,
                    verrouille: Boolean(timeframe.verrouille),
                    decision_finale: fusion.decision,
                    confiance_globale: fusion.confiance,
                    gold_signal: gold,
                    groupe_prive_v2_interne: groupe,
                    technique,
                    note: "Signal interne indépendant. Ne reproduit pas le code des indicateurs privés TradingView.",
                    date: maintenantIso()
                });
            } catch (erreurTimeframe) {
                signaux.push({
                    ok: false,
                    key: timeframe.key || intervalle,
                    label: timeframe.label || intervalle,
                    ...
                    confiance_globale: 0,
                    message: "Signal interne indisponible pour ce timeframe.",
                    details: erreurTimeframe.message,
                    note: "Aucun signal privé n'est appelé. Échec du calcul serveur interne.",
                    date: maintenantIso()
                });
            }
        }
        ...
        res.json({
            ok: true,
            message: "Signaux internes multi-timeframes calculés.",
            avertissement: "Ces signaux sont des approximations internes. Ils ne copient pas et ne garantissent pas les indicateurs privés Gold signal ou Groupe privé V2.",
            signaux,
            date: maintenantIso()
        });
    } catch (erreur) {
        res.status(500).json({
            ok: false,
            message: "Erreur lors du calcul des signaux internes multi-timeframes.",
            details: erreur.message,
            date: maintenantIso()
        });
    }
...
/* ============================================================
   Paiement accès symboles / indicateurs
   Route appelée par index.html : POST /api/paiement/checkout

   Sécurité :
   - aucune donnée de carte bancaire ou de compte bancaire n'est traitée ici ;
   - le serveur prépare seulement une demande ;
   - le paiement réel doit être traité par une passerelle conforme.
============================================================ */

const PAIEMENT_MODE_SIMULATION = String(process.env.PAIEMENT_MODE_SIMULATION || "true").toLowerCase() !== "false";
const PAIEMENT_DEVISE = String(process.env.PAIEMENT_DEVISE || "CAD").toUpperCase();
const PAIEMENT_MONTANT_MIN_CENTS = Math.max(100, Math.round(Number(process.env.PAIEMENT_MONTANT_MIN_CENTS || 100)));
const PAIEMENT_MONTANT_MAX_CENTS = Math.max(PAIEMENT_MONTANT_MIN_CENTS, Math.round(Number(process.env.PAIEMENT_MONTANT_MAX_CENTS || 500000)));
const PAIEMENT_SUCCESS_URL = process.env.PAIEMENT_SUCCESS_URL || null;
const PAIEMENT_CANCEL_URL = process.env.PAIEMENT_CANCEL_URL || null;

function nettoyerCodePaiement(valeur, defaut = "") {
...
function libelleProduitPaiement(typeProduit) {
    const type = String(typeProduit || "pack_symbole_indicateur").toLowerCase();

    if (type === "symbole") return "Accès symbole";
    if (type === "indicateur") return "Accès indicateur";
    if (type === "forfait" || type === "pack" || type === "pack_symbole_indicateur") {
        return "Accès symbole + indicateur";
    }

    return "Accès Expert Trading";
}

function montantPaiementCents(typeProduit) {
    const type = String(typeProduit || "pack_symbole_indicateur").toLowerCase();

    const montantSymbole = Number(process.env.PRIX_SYMBOLE_CENTS || 499);
    const montantIndicateur = Number(process.env.PRIX_INDICATEUR_CENTS || 499);
    const montantPack = Number(process.env.PRIX_PACK_CENTS || 799);

    if (type === "symbole") return Math.max(100, Math.round(montantSymbole));
    if (type === "indicateur") return Math.max(100, Math.round(montantIndicateur));

    return Math.max(100, Math.round(montantPack));
}

function convertirMontantPaiementLibreCents(body, typeProduit) {
    const montantCentsBrut = Number(body?.montant_cents);

    if (Number.isFinite(montantCentsBrut) && montantCentsBrut > 0) {
        return Math.round(montantCentsBrut);
    }

    const montantBrut = Number(String(body?.montant || body?.amount || "").replace(",", "."));

    if (Number.isFinite(montantBrut) && montantBrut > 0) {
        return Math.round(montantBrut * 100);
    }

    return montantPaiementCents(typeProduit);
}

function validerMontantPaiementCents(montantCents) {
    const montant = Math.round(Number(montantCents));

    if (!Number.isFinite(montant)) {
        const erreur = new Error("Montant de paiement invalide.");
        erreur.httpStatus = 400;
        throw erreur;
    }

    if (montant < PAIEMENT_MONTANT_MIN_CENTS) {
        const erreur = new Error("Montant trop faible. Minimum autorisé : " + (PAIEMENT_MONTANT_MIN_CENTS / 100).toFixed(2) + " " + PAIEMENT_DEVISE + ".");
        erreur.httpStatus = 400;
        throw erreur;
    }

    if (montant > PAIEMENT_MONTANT_MAX_CENTS) {
        const erreur = new Error("Montant trop élevé. Maximum autorisé : " + (PAIEMENT_MONTANT_MAX_CENTS / 100).toFixed(2) + " " + PAIEMENT_DEVISE + ".");
        erreur.httpStatus = 400;
        throw erreur;
    }
...
async function initialiserTablePaiements() {
    verifierDbConfiguree();

    await db.query(`
        CREATE TABLE IF NOT EXISTS paiement_acces (
            id SERIAL PRIMARY KEY,
            utilisateur_id INTEGER,
            username VARCHAR(100),
            type_produit VARCHAR(80) NOT NULL,
            mode_paiement VARCHAR(40) NOT NULL,
            symbole VARCHAR(120),
            symbole_libelle VARCHAR(255),
            indicateur VARCHAR(120),
            indicateur_libelle VARCHAR(255),
            intervalle VARCHAR(50),
            categorie_analyse VARCHAR(100),
            devise VARCHAR(10) DEFAULT 'CAD',
            montant_cents INTEGER NOT NULL,
            statut VARCHAR(40) DEFAULT 'prepare',
...
async function creerSessionStripeCheckout(demande) {
    if (!process.env.STRIPE_SECRET_KEY) {
        return null;
    }

    const successUrl = demande.retour_succes || PAIEMENT_SUCCESS_URL;
    const cancelUrl = demande.retour_annulation || PAIEMENT_CANCEL_URL;

    if (!successUrl || !cancelUrl) {
        const erreur = new Error("PAIEMENT_SUCCESS_URL et PAIEMENT_CANCEL_URL doivent être configurées, ou envoyées par index.html.");
        erreur.httpStatus = 500;
        throw erreur;
    }
    ...
    const methodes = String(demande.mode_paiement || "carte").toLowerCase().includes("banc")
        ? ["card"]
        : ["card"];

    methodes.forEach((methode, index) => {
        params.append(`payment_method_types[${index}]`, methode);
    });

    params.set("line_items[0][quantity]", "1");
    params.set("line_items[0][price_data][currency]", String(demande.devise || PAIEMENT_DEVISE).toLowerCase());
    params.set("line_items[0][price_data][unit_amount]", String(demande.montant_cents));
    params.set("line_items[0][price_data][product_data][name]", demande.nom_produit);
    params.set("line_items[0][price_data][product_data][description]", demande.description);

    params.set("metadata[type_produit]", demande.type_produit);
    params.set("metadata[symbole]", demande.symbole || "");
    params.set("metadata[indicateur]", demande.indicateur || "");
    params.set("metadata[username]", demande.username || "");
    params.set("metadata[montant_cents]", String(demande.montant_cents));
    ...
    try {
        ...
        throw new Error("Réponse Stripe non JSON : " + texte);
    }

    if (!response.ok) {
        const messageStripe = data?.error?.message || texte || "Erreur Stripe inconnue.";
        const erreur = new Error("Stripe Checkout a refusé la demande : " + messageStripe);
        erreur.httpStatus = response.status;
        throw erreur;
    }
...
app.post("/api/paiement/checkout", async (req, res) => {
    try {
        const body = req.body || {};

        const typeProduit = nettoyerCodePaiement(body.type_produit || body.produit || "pack_symbole_indicateur");
        const modePaiement = nettoyerCodePaiement(body.mode_paiement || body.methode_paiement || "carte");
        const symbole = nettoyerCodePaiement(body.symbole || body.actif || "");
        const indicateur = nettoyerCodePaiement(body.indicateur || "");
        const symboleLibelle = nettoyerCodePaiement(body.symbole_libelle || symbole);
        const indicateurLibelle = nettoyerCodePaiement(body.indicateur_libelle || indicateur);
        const intervalle = nettoyerCodePaiement(body.intervalle || "");
        const categorieAnalyse = nettoyerCodePaiement(body.categorie_analyse || body.categorieAnalyse || "technique");
        const devise = nettoyerCodePaiement(body.devise || PAIEMENT_DEVISE).toUpperCase() || PAIEMENT_DEVISE;
        ...
        if (typeProduit !== "indicateur" && !symbole) {
            return res.status(400).json({
                ok: false,
                message: "Symbole requis pour ce type de produit.",
                date: maintenantIso()
            });
        }

        if (typeProduit !== "symbole" && !indicateur) {
            return res.status(400).json({
                ok: false,
                message: "Indicateur requis pour ce type de produit.",
                date: maintenantIso()
            });
        }

        const montantCents = validerMontantPaiementCents(
            convertirMontantPaiementLibreCents(body, typeProduit)
        );
        const nomProduit = libelleProduitPaiement(typeProduit);
        const description = [symboleLibelle, indicateurLibelle].filter(Boolean).join(" / ") || nomProduit;

        const demande = {
            type_produit: typeProduit,
            mode_paiement: modePaiement,
            symbole,
            symbole_libelle: symboleLibelle,
            indicateur,
            indicateur_libelle: indicateurLibelle,
            intervalle,
            ...
            description,
            retour_succes: body.retour_succes || PAIEMENT_SUCCESS_URL,
            retour_annulation: body.retour_annulation || PAIEMENT_CANCEL_URL,
            username: req.user?.username || null,
            utilisateur_id: req.user?.id || null,
            source: body.source || "api_paiement_checkout"
        };

        let sessionPaiement = null;
        let statut = "simulation";

        if (!PAIEMENT_MODE_SIMULATION) {
            sessionPaiement = await creerSessionStripeCheckout(demande);
            statut = sessionPaiement?.fournisseur || "prepare";
        }

        if (db) {
            await initialiserTablePaiements();

            const insertion = await db.query(`
                INSERT INTO paiement_acces (
                    utilisateur_id,
                    username,
                    type_produit,
                    mode_paiement,
            ...
            `, [
                ...
                demande.symbole,
                demande.symbole_libelle,
                demande.indicateur,
                demande.indicateur_libelle,
                demande.intervalle,
                demande.categorie_analyse,
                demande.devise,
                demande.montant_cents,
                statut,
                sessionPaiement?.session_id || null,
                sessionPaiement?.checkout_url || null,
                demande
            ]);

            demande.id_paiement = insertion.rows[0].id;
            demande.date_creation = insertion.rows[0].date_creation;
        }

        return res.json({
            ok: true,
            message: sessionPaiement?.checkout_url
                ? "Session de paiement créée."
                : "Demande de paiement préparée en mode simulation.",
            simulation: !sessionPaiement?.checkout_url,
            fournisseur: sessionPaiement?.fournisseur || "simulation",
            checkout_url: sessionPaiement?.checkout_url || null,
            session_id: sessionPaiement?.session_id || null,
            paiement: demande,
            securite: "Aucune donnée bancaire n'est traitée par cette API. Le paiement réel doit passer par une passerelle conforme.",
            date: maintenantIso()
        });

    } catch (erreur) {
        console.error("Erreur /api/paiement/checkout :", erreur);

        return res.status(erreur.httpStatus || 500).json({
            ok: false,
            message: "Erreur lors de la création de la session de paiement.",
            details: erreur.message,
            stripe_configuree: Boolean(process.env.STRIPE_SECRET_KEY),
            mode_simulation: PAIEMENT_MODE_SIMULATION,
            date: maintenantIso()
        });
    }
});

app.get("/api/paiement/historique", async (req, res) => {
    try {
        await initialiserTablePaiements();
        ...
        res.status(erreur.httpStatus || 500).json({
            ok: false,
            message: "Impossible de lire l'historique des paiements.",
            details: erreur.message,
            date: maintenantIso()
        });
    }
